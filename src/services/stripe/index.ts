...
export const createStripeCustomer = async (input: { name?: string; email: string }) => {
...
export const createCheckoutSession = async (
 ...
 try {
  ...
  const session = await stripe.billingPortal.sessions.create({
   ...
   return_url: `http://localhost:3000/dashboard/configuracoes/assinatura`,
   flow_data: {
    type: "subscription_update_confirm",
    after_completion: {
     type: "redirect",
     redirect: {
      return_url: `http://localhost:3000/dashboard/configuracoes/assinatura?success=true`,
     },
    },
...
export const getPlanByPrice = (priceId: string) => {
 ...
 const planKey = Object.keys(plans).find((key) => plans[key].priceId === priceId) as
  | keyof Plans
  | undefined;
...
export const getUserCurrentPlan = async (userId: string) => {
  ...
  if (!user || !user.stripePriceId) {
    throw new Error("Usuário não encontrado ou não possui um plano associado.");
  }
